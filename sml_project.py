...
Original file is located at
    https://colab.research.google.com/drive/1rpMjA_sC5Qpw24OdMop_H-sqdNUCWvHM
"""
...
X = daily_mean.drop(columns=["usage"])
y = daily_mean["usage"]
...
# Create a function to make predictions using the chosen model
def predict_usage(model, X_new):
    return model.predict(X_new.reshape(1, -1))[0]

# Create the Streamlit app
st.title("Steel Industry Usage Predictor")

# Create a dropdown menu to select the machine learning model
model_selectbox = st.sidebar.selectbox("Select a Machine Learning Model", ["Linear Regression", "Decision Tree", "Random Forest", "Support Vector Machine", "Neural Network"])

# Create a slider to select a date for prediction
date_slider = st.slider("Select a Date for Prediction", min_value=X.index.min().timestamp(), max_value=X.index.max().timestamp(), value=X.index.min().timestamp(), step=86400)

# Get the corresponding features for the selected date
X_new = np.array(X.loc[pd.to_datetime(date_slider, unit="s")])

# Get the selected model
if model_selectbox == "Linear Regression":
    model = linreg
elif model_selectbox == "Decision Tree":
    model = dtr
elif model_selectbox == "Random Forest":
    model = rfr
elif model_selectbox == "Support Vector Machine":
    model = svm
else:
    model = nn

# Make the prediction using the selected model
prediction = predict_usage(model, X_new)

# Display the prediction
st.write(f"Predicted usage on {pd.to_datetime(date_slider, unit='s').date()}: {prediction:.2f} units")